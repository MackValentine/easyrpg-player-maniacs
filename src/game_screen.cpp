...
void Game_Screen::UpdateSandstorm() {
	...
	for (size_t i = 2; i < particles.size(); ++i) {
		...
		if (p.t > 0) {
			...
			p.x = static_cast<int>(d * c * 2.0f) * Player::Screen_Width / Player::Screen_Width + Player::Screen_Width / 2;
			p.y = static_cast<int>(d * s) * Player::Screen_Height / Player::Screen_Height;

			p.alpha = 180;
			p.vx = 0.0;
			p.vy = 0.0;
			p.ax = c * 2.0f * Player::Screen_Width / Player::Screen_Width;
			p.ay = s * 2.0f * Player::Screen_Height / Player::Screen_Height;
		}
	}
}
