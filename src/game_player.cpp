...
void Game_Player::UpdateScroll(int amount, bool was_jumping) {
	...
	int xx = (Player::Screen_Width / 16) - 20;
	int yy = (Player::Screen_Height / 16) - 15;

	auto dx = ((GetX()-xx) * SCREEN_TILE_SIZE) - Game_Map::GetPositionX() - GetPanX();
	auto dy = ((GetY()-yy) * SCREEN_TILE_SIZE) - Game_Map::GetPositionY() - GetPanY();
