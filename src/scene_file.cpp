...
std::unique_ptr<Sprite> Scene_File::MakeBorderSprite(int y) {
	auto bitmap = Bitmap::Create(Player::Screen_Width, 8, Cache::System()->GetBackgroundColor());
...
std::unique_ptr<Sprite> Scene_File::MakeArrowSprite(bool down) {
	...
	sprite->SetBitmap(bitmap);
	sprite->SetX(Player::Screen_Width / 2 - 8);
	sprite->SetY(down ? Player::Screen_Height - 8 : 32);
	return sprite;
}
