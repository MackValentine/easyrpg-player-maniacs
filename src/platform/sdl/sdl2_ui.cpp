...
void Sdl2Ui::ToggleZoom() {
	...
	last_display_mode.zoom = std::min(w / Player::Screen_Width, h / Player::Screen_Height);
	...
	if ((max_mode.h < Player::Screen_Height * current_display_mode.zoom) ||
		(max_mode.w < Player::Screen_Width * current_display_mode.zoom)) {
...
void Sdl2Ui::UpdateDisplay() {
	...
	if (window.size_changed && window.width > 0 && window.height > 0) {
		...
		float want_aspect = (float)Player::Screen_Width / Player::Screen_Height;
		float real_aspect = width_float / height_float;

		if (scaling_mode == ScalingMode::Integer) {
			// Integer division on purpose
			if (want_aspect > real_aspect) {
				window.scale = static_cast<float>(window.width / Player::Screen_Width);
			} else {
				window.scale = static_cast<float>(window.height / Player::Screen_Height);
			}

			viewport.w = static_cast<int>(ceilf(Player::Screen_Width * window.scale));
			viewport.x = (window.width - viewport.w) / 2;
			viewport.h = static_cast<int>(ceilf(Player::Screen_Height * window.scale));
			...
			window.scale = height_float / Player::Screen_Height;
			viewport.y = 0;
		...
		if (scaling_mode == ScalingMode::Bilinear && window.scale > 0.f) {
			...
			sdl_texture_scaled = SDL_CreateTexture(sdl_renderer, texture_format, SDL_TEXTUREACCESS_TARGET,
			   static_cast<int>(ceilf(window.scale)) * Player::Screen_Width, static_cast<int>(ceilf(window.scale)) * Player::Screen_Height);
