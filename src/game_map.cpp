...
void Game_Map::SetupFromSave(
	...
	SetEncounterRate(map_info.encounter_steps);
...
void Game_Map::PrepareSave(lcf::rpg::Save& save) {
	...
	if (save.map_info.encounter_steps == GetOriginalEncounterRate()) {
...
void Game_Map::Parallax::ResetPositionX() {
	...
	if (!params.scroll_horz && !LoopHorizontal()) {
		if (GetWidth() > Player::Screen_Width/ TILE_SIZE && parallax_width > Player::Screen_Width) {
			const int w = (GetWidth() - Player::Screen_Width/ TILE_SIZE) * TILE_SIZE;
...
void Game_Map::Parallax::ResetPositionY() {
	...
	if (!params.scroll_vert && !Game_Map::LoopVertical()) {
		if (GetHeight() > Player::Screen_Height/ TILE_SIZE && parallax_height > Player::Screen_Height) {
			const int h = (GetHeight() - Player::Screen_Height / TILE_SIZE) * TILE_SIZE;
